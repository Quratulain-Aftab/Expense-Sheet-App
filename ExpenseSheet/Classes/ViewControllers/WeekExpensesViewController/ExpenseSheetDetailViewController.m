...
@implementation ExpenseSheetDetailViewController
{
    ...
    NSString *previousCustomer;
    NSString *previousProject;
}
...
- (IBAction)addButtonAction:(id)sender {
    ...
    if(self.dataSource.count>0)
    {
        previousCustomer=[self.dataSource.lastObject valueForKey:CustomerName];
        previousProject=[self.dataSource.lastObject valueForKey:ProjectName];
        self.mode=4;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if([segue.identifier isEqualToString:@"detailsegue"])
    {
        ...
        if(previousCustomer)
        {
            detailVC.previousCustomer=previousCustomer;
            detailVC.previousProject=previousProject;
        }
    }
}
