...
- (void)viewDidLoad
{
   ...
   if([[mySharedDefaults objectForKey:@"task"] isEqualToString:@"OpenCreateSheetView"])
   {
...
-(void)adjustTable
{
    ...
    {
        self.duethisWeekLabel.hidden=NO;
        self.CurrentWeekTable.hidden=false;
         self.noExpenseSheetMainViewLabel.hidden=YES;

    }
    else
    {
        
        self.duethisWeekLabel.hidden=YES;
        self.noExpenseSheetMainViewLabel.hidden=NO;
        self.CurrentWeekTable.hidden=YES;
        NSUserDefaults *mySharedDefaults = [[NSUserDefaults alloc] initWithSuiteName:AppGroupName];
        
        // Use the shared user defaults object to update the user's account
        [mySharedDefaults setObject:nil forKey:@"Description"];
        [mySharedDefaults setObject:nil forKey:@"Duration"];
...
- (IBAction)createButtonAction:(id)sender
{
    ...
    NSDate *date1=[self getWeekStartDateFromDate:self.createDatePicker.date];
    
    if([self isDateBelongsToCurrentWeek:date1])
    {
        ...
        [mySharedDefaults setObject:[df stringFromDate:date1] forKey:@"Duration"];
...
- (IBAction)dateChanged:(id)sender {
    ...
    [components setWeekday:[[[Utilities shareManager]getUpdatedSettingsForString:WeekStartDayKey]integerValue]]; // 1: Sunday
