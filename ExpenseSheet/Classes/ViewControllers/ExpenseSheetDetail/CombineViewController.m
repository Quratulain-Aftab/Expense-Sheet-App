...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    {
         [_delegate sendDataToPreviousControllerWhereCustomer:self.selectedCustomer andProject:self.selectedProject];
...
{
    //step 1:  initially done button is disable
...
- (IBAction)pickerViewDoneButtonAction:(id)sender {
    if([self.pickerviewTitleLabel.text isEqualToString:@"Select Customer"])
    {
       NSInteger  row = [self.pickerView selectedRowInComponent:0];
        ...
        // step 2: done button will be enable only if user successfully updated customer and project name atleast once
