...
-(void)makeUIAdjustments
{
    ...
    [self getStartDateOfSheet];
    
    NSDateFormatter * df = [[NSDateFormatter alloc] init];
    [df setDateFormat:@"MM-dd-yyyy"]; // from here u can change format..
    [self.dateButton setTitle:[df stringFromDate:startDate] forState:UIControlStateNormal]
    ;
...
-(void)configureView:(NSInteger)mode
{
    if(mode==1)
    {
    ...
    else if (mode==4)
        {
            ...
            self.projectLabel
            .hidden=YES;
            [self.customerButton setTitle:self.previousCustomer forState:UIControlStateNormal];
             [self.projectButton setTitle:self.previousProject forState:UIControlStateNormal];
...
- (IBAction)SaveButtonAction:(id)sender
{
    ...
    if(self.mode==1 || self.mode==4)
    {
        ...
        if(self.previousProject)
        {
        [newSheetDetail setValue:self.previousProject forKey:ProjectName];
        }
        if(self.previousCustomer)
        {
        [newSheetDetail setValue:self.previousCustomer forKey:CustomerName];
        }
        if(_image)
        {
            [self saveImage];
        ...
        {
             [newSheetDetail setValue:startDate forKey:ExpenseDate];
        }
...
- (IBAction)customerButtonAction:(id)sender {
    ...
    UIAlertAction *customer1Action = [UIAlertAction actionWithTitle:@"Smart IS" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        ;
        self.previousCustomer=@"Smart IS";
    ...
    UIAlertAction *customer4Action = [UIAlertAction actionWithTitle:@"Oracular Inc" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        ;
        self.previousCustomer=@"Oracular Inc";
...
- (IBAction)projectButtonAction:(id)sender {
    ...
    UIAlertAction *Ok = [UIAlertAction actionWithTitle:@"Project name 2" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        ;
        self.previousProject=@"Project name 2";
    ...
    UIAlertAction *yes = [UIAlertAction actionWithTitle:@"Project name 3" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        ;
        self.previousProject=@"Project name 3";
...
- (IBAction)selectDateButtonAction:(id)sender
{
  ...
  // NSDate *minDate = [[NSCalendar currentCalendar] dateByAddingComponents:comps toDate:startDate  options:0];
    [comps setDay:7];
    NSDate *maxDate=[[NSCalendar currentCalendar] dateByAddingComponents:comps toDate:startDate options:0];
...
{
    [self performSelector:@selector(callWithDelay) withObject:nil afterDelay:1.5];
}
-(void)callWithDelay
{
...
{
    self.previousCustomer=customerName;
    self.previousProject=projectName;
    
}
-(void)getStartDateOfSheet
{
    NSManagedObjectContext *managedObjectContext = [self managedObjectContext];
    ...
    startDate=[[results objectAtIndex:0] valueForKey:StartDate];
